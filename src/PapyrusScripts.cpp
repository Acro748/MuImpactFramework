...
namespace Mus {
    namespace Papyrus {
        ...
        void RegisterSoundOnActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand, RE::BGSSoundDescriptorForm* sound, bool SecondSound)
        {
            if (!actor || !sound)
                return;
            ImpactManager::GetSingleton().AddSound(actor, LeftHand, sound, SecondSound);
        }
        void UnRegisterSoundOnActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand, RE::BGSSoundDescriptorForm* sound, bool SecondSound)
        {
            if (!actor || !sound)
                return;
            ImpactManager::GetSingleton().RemoveSound(actor, LeftHand, sound, SecondSound);
        }
        void UnRegisterAllSoundActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand)
        {
            if (!actor)
                return;
            ImpactManager::GetSingleton().RemoveSound(actor, LeftHand);
        }
        ...
        void RegisterEffectShaderOnActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand, RE::TESEffectShader* effectShader)
        {
            if (!actor || !effectShader)
                return;
            ImpactManager::GetSingleton().AddEffectShader(actor, LeftHand, effectShader);
        }
        void UnRegisterEffectShaderOnActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand, RE::TESEffectShader* effectShader)
        {
            if (!actor || !effectShader)
                return;
            ImpactManager::GetSingleton().RemoveEffectShader(actor, LeftHand, effectShader);
        }
        void UnRegisterAllEffectShaderActor(RE::StaticFunctionTag*, RE::Actor* actor, bool LeftHand)
        {
            if (!actor)
                return;
            ImpactManager::GetSingleton().RemoveEffectShader(actor, LeftHand);
        }
        ...
        bool RegisterPapyrusFunctions(RE::BSScript::IVirtualMachine* vm) {
            ...
            vm->RegisterFunction("RegisterSoundOnActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), RegisterSoundOnActor);
            vm->RegisterFunction("UnRegisterSoundOnActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), UnRegisterSoundOnActor);
            vm->RegisterFunction("UnRegisterAllSoundActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), UnRegisterAllSoundActor);

            vm->RegisterFunction("RegisterEffectShaderOnActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), RegisterEffectShaderOnActor);
            vm->RegisterFunction("UnRegisterEffectShaderOnActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), UnRegisterEffectShaderOnActor);
            vm->RegisterFunction("UnRegisterAllEffectShaderActor", SKSE::PluginDeclaration::GetSingleton()->GetName().data(), UnRegisterAllEffectShaderActor);
            return true;
        }
    }
}  // namespace Mus